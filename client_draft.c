        ...
        for (int i = 0; i < FD_SETSIZE; i++)
        {
            ...
            if (FD_ISSET(i, &ready_sockets))
            {
                ...
                else
                {
                    ...
                    char *sub_hash = strstr(buffer, "[HASH]:");
                    if (sub != NULL)
                    {
                    ...
                    {
                        // receiving file size
                        // unsigned char hash_code[SHA_DIGEST_LENGTH];
                    ...
                    else if(sub_hash != NULL){
                        printf("hashcode : %s", buffer);
                    }
                    else
                    {
                        // possible hashcode
                        printf("possible hashcode\n");
                        printf("%s", buffer);
                    }
                    // printf("\n%s\n", buffer);
...
void send_file(char ip[], int des_port, char filename[SIZE])
{
    ...
    while (file_size > 0 && (bytes_read = fread(buffer, sizeof(char), sizeof(buffer), src_file)) > 0)
    {
        printf("content: %s\n", buffer);
    ...
    // char end_download[SIZE];
    // sprintf(end_download, "[HASH]:%s", hash);
    // int last = send(des_sock, hash, sizeof(hash), 0);
    // if(last < 0){
    //     printf("error in sending hash code \n");
    // }
    // printf("\nafter sending, hash code from sender: %s", hash);
    fclose(src_file);
    printf("\ndone read file\n");
    return ;
}
...
void receive_file(int src_sockfd, char filename[], long file_size)
{
    ...
    char hash_str[SIZE];
    // int hash_length = recv(src_sockfd, hash_str, sizeof(hash_str), 0);
    char finished_download[SIZE];
    strcpy(finished_download, "Download completed");
    printf("\n%s", finished_download);

    // getchar();
    unsigned char hash_build[SHA_DIGEST_LENGTH];
    SHA1_Final(hash_build, &ctx);

    printf("\nbuilt hash from receiver: %s", hash_build);
    unsigned char hash[SHA_DIGEST_LENGTH];
    // int hash_len = recv(src_sockfd, hash, sizeof(hash), 0);
    // if(hash_len < 0)
    // {
    //     printf("\nerror in receiving hash code");
    // }else{
    //     if(strcmp(hash, hash_build) == 0){
    //         printf("file transfer correct");
    //     }else{
