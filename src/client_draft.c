...
int handle_download(char ip[INET_ADDRSTRLEN], int port);
void *receive_thread(void *server_fd);
void *receiving(int server_fd);
void sending(char ip[INET_ADDRSTRLEN], int sender_port, char file_name[SIZE / 2]);
...
int main(int argc, char *argv[])
{
    ...
    server_addr.sin_port = htons(port_of_server);
    ...
    while (1)
    {
        ...
        case 2:
            ...
            if (success == true)
            {
                int serv_port_of_client = atoi(server_port_of_client);
                int server_fd;
                server_fd = handle_download(ip, serv_port_of_client);
                while (1)
                {
                    int sub_choice;
                    submenu();
                    ...
                    else if (sub_choice == 2)
                    {
                        ...
                        printf("enter file name you ");
                        scanf("%s", file_name);
                        // strcpy(file_name_inserted, filename);
                        sending(ip, serv_port_of_client, file_name);
                        // break;
                        continue;
                    }
                    else
...
// set up server in client side
int handle_download(char ip[], int port)
{
        ...
        for (int i = 0; i < FD_SETSIZE; i++)
        {
            ...
            if (FD_ISSET(i, &ready_sockets))
            {
                ...
                else
                {
                    ...
                    else if (strstr(buffer, "[FILE SIZE]:") != NULL)
                    {
                        // receiving file size
...
void sending(char ip[], int sender_port, char file_name[])
{
    char buffer[SIZE] = {0};
...
void receive_file(int src_sockfd, char filename[], long file_size)
{
    ...
    strcpy(finished_download, "done write file");
    printf("\n%s", finished_download);
